# Contributing to @bakissation/houwiya

Thanks for your interest! This library validates and parses Algerian identifiers used in onboarding, KYC, invoicing, and payments — correctness and backward compatibility matter more than features.
...
```bash
git clone https://github.com/bakissation/houwiya.git
cd houwiya
npm install
npm run build
npm test
...
- On merge per channel: **`dev` → alpha**, **`staging` → beta**, **`main` → stable**. Notes go to [GitHub Releases](https://github.com/bakissation/houwiya/releases).
